...
from ..time_diff import multi_session_time_slice_diffs, plot_tsdiffs
...
from .check_preprocessing import plot_registration, plot_spm_motion_parameters
...
def get_nipype_report_filename(output_files_or_dir):
    if isinstance(output_files_or_dir, str):
        if os.path.isdir(output_files_or_dir):
            return os.path.join(output_files_or_dir, "_report/report.rst")
        elif os.path.isfile(output_files_or_dir):
            return get_nipype_report_filename(os.path.dirname(output_files_or_dir))
        else:
            raise OSError("%s is neither a file nor directory!" % output_files_or_dir)
    else:
...
def generate_preproc_undergone_docstring(
    ...
    if tools_used is None:
        tools_used = (
            ...
            "preprocessing functional and anatomical MRI data." % (PYAFFINEPREP_URL)
        )
    ...
    preproc_undergone += (
        "<br>For each subject, the following preprocessing steps have been done:"
    )
    ...
    if details_filename is not None:
        preproc_undergone += (
            " <a href=%s>See complete configuration used for preprocessing here</a>"
        ) % os.path.basename(details_filename)
    preproc_undergone += "</ul>"
...
def export_report(src, tag="", make_archive=True):
    ...
    def ignore_these(folder, files):
        return [
            f
            for f in files
            if (os.path.isfile(os.path.join(folder, f)) and not check_extension(f))
        ]
...
def generate_registration_thumbnails(
    ...
    # normalized image
    outline = os.path.join(output_dir, "%s_on_%s_outline.png" % (target[1], source[1]))
...
def generate_tsdiffana_thumbnail(
    image_files, sessions, subject_id, output_dir, results_gallery=None, tooltips=None
):
    ...
    output_filename_template = os.path.join(output_dir, "tsdiffana_plot_{0}.png")
    output_filenames = [output_filename_template.format(i) for i in range(len(figures))]
    ...
    for output_filename, tooltip in zip(output_filenames, tooltips):
        ...
        thumbnail.img = img(
            src=os.path.basename(output_filename), height="250px", width="600px"
        )
        thumbnail.description = "tsdiffana ({0} sessions)".format(len(sessions))
...
def generate_stc_thumbnails(
    ...
    for session_id, o_bold, stc_bold in zip(sessions, original_bold, st_corrected_bold):
        stc_ts = _get_time_series_from_voxel(stc_bold, voxel)
        o_ts = _get_time_series_from_voxel(o_bold, voxel)
        output_filename = os.path.join(output_dir, "stc_plot_%s.png" % session_id)
        pl.figure()
        pl.plot(o_ts, "o-")
        pl.hold("on")
        pl.plot(stc_ts, "s-")
        ...
        pl.title(
            "session %s: STC QA for voxel (%s, %s, %s)"
...
def generate_subject_preproc_report(
    ...
    def finalize_report():
        ...
        if parent_results_gallery:
            commit_subject_thumnbail_to_parent_gallery(
                final_thumbnail, subject_id, parent_results_gallery
            )
    ...
    report_preproc_filename = os.path.join(output_dir, "report_preproc.html")
    report_filename = os.path.join(output_dir, "report.html")

    # copy css and js stuff to output dir
    for js_file in glob.glob(os.path.join(ROOT_DIR, "js/*.js")):
    ...
    results_gallery = ResultsGallery(
        loader_filename=loader_filename, title="Report for subject %s" % subject_id
    )
    ...
    # html markup
    preproc = get_subject_report_preproc_html_template().substitute(
        conf_path=conf_path,
    ...
    main_html = get_subject_report_html_template().substitute(
        conf_path=conf_path, start_time=time.ctime(), subject_id=subject_id
    )
    ...
    # generate stc thumbs
    if did_slice_timing and original_bold is not None and st_corrected_bold is not None:
    ...
    if tsdiffana:
        generate_tsdiffana_thumbnail(
            func, sessions, subject_id, output_dir, results_gallery=results_gallery
        )
...
def make_nipype_execution_log_html(
    nipype_output_files, node_name, output_dir, brain_name="", progress_logger=None
):
    brain_name = brain_name.replace(" ", "_")  # fix forissue 169
    execution_log = get_nipype_report(get_nipype_report_filename(nipype_output_files))
    ...
    open(execution_log_html_filename, "w").write(execution_log)

    if progress_logger:
        progress_logger.log("<b>%s</b><br/><br/>" % node_name)
