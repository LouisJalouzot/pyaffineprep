...
def _correct_voxel_samp(affine, samp):
    ...
    return 1.0 * np.array(samp) / np.sqrt(np.sum(affine[:3, :3] ** 2, axis=0))
...
def make_sampled_grid(shape, samp=[1.0, 1.0, 1.0], magic=True):
    """
    ...
    # sanitize input
    assert len(shape) == 3, ("Expected triplet of 3 integers, got %s") % shape
    ...
    assert samp.ndim == 1 and len(samp) == 3, (
        "samp must be float of triple of floats, got %s"
    ) % samp
    ...
    if magic:
        iran = 0
        grid = []
        for z in np.arange(1.0, shape[2] - samp[2], samp[2]):
            for y in np.arange(1.0, shape[1] - samp[1], samp[1]):
                for x in np.arange(1.0, shape[0] - samp[0], samp[0]):
                    # doping
                    iran = (iran + 1) % 97
        ...
        return np.mgrid[
            1 : shape[0] - samp[0] : samp[0],
            1 : shape[1] - samp[1] : samp[1],
            1 : shape[2] - samp[2] : samp[2],
        ].reshape((3, -1))
...
def trilinear_interp(f, shape, x, y, z):
    ...
    offset = np.asarray(
        ix - 1 + shape[0] * (iy - 1 + shape[1] * (iz - 1)), dtype=np.integer
    )
...
def joint_histogram(ref, src, grid=None, samp=None, M=np.eye(4), bins=(256, 256)):
    ...
    if grid is None:
        ...
        assert is_niimg(ref), ("grid is None, expected niimg for ref, got %s") % type(
            ref
        )

        # create sampled grid for ref img
        grid = make_sampled_grid(ref.shape, samp=_correct_voxel_samp(ref.affine, samp))

        # interpolate ref on sampled grid
        ref = trilinear_interp(
            ref.get_fdata().ravel(order="F"), ref.shape, grid[0], grid[1], grid[2]
        )
    else:
