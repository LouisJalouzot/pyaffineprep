...
def make_dataset(
    ...
    for i in range(n_subjects):
        ...
        nibabel.save(create_random_image(ndim=3), subject_data["anat"])
        ...
        for j in range(n_sessions):
            session_dir = os.path.join(subject_data_dir, "session%03i" % (j + 1))
            ...
            if threeD_filenames:
                for k in range(n_scans):
                    func_filename = os.path.join(
                        session_dir, "func%03i.%s" % (j + 1, ext)
                    )
                    nibabel.save(create_random_image(ndim=3), func_filename)
                    sfunc.append(func_filename)
                subject_data["func"].append(sfunc)
            else:
                func_filename = os.path.join(session_dir, "func.%s" % ext)
                nibabel.save(
                    create_random_image(ndim=4, n_scans=n_scans), func_filename
                )
...
def _make_sd(
    ...
    if not func_filenames is None:
        ...
        for sess in range(n_sessions):
            sess_dir = (
                DATA_DIR
                if not make_sess_dirs
                else os.path.join(DATA_DIR, "session%i" % sess)
            )
            if not os.path.exists(sess_dir):
                os.makedirs(sess_dir)
            func_filename = "%s/func%s%s" % (
                sess_dir,
                "_sess_%i_" % sess if (n_sessions > 1 and unique_func_names) else "",
                ext,
            )
            _save_img(func[sess], func_filename)
            func_filenames.append(func_filename)

    sd = SubjectData(anat=anat_filename, func=func_filenames, output_dir=output_dir)
    return sd
    return sd
