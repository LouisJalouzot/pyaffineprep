...
def test_fwhm2sigma():
    ...
    for _fwhm in fwhm:
        numpy.testing.assert_array_equal(
            fwhm2sigma(_fwhm), np.array(_fwhm) / np.sqrt(8.0 * np.log(2))
        )
    for j in range(3):
        _fwhm = fwhm[j:]
        numpy.testing.assert_array_equal(
...
def test_sigma2sigma():
    ...
    for _sigma in sigma:
        numpy.testing.assert_array_equal(
            sigma2fwhm(_sigma), np.array(_sigma) * np.sqrt(8.0 * np.log(2))
        )
    for j in range(3):
        _sigma = sigma[j:]
        numpy.testing.assert_array_equal(
...
def test_smooth_image_for_3D_vol():
    ...
    svol = smooth_image(vol, [5, 7, 11.0])
...
def test_smooth_image_for_4D_film():
    ...
    sfilm = smooth_image(film, [5, 7.0, 11])
