...
from ..time_diff import multi_session_time_slice_diffs, plot_tsdiffs, time_slice_diffs
...
def make_test_data(n_scans=3):
    ...
    film[-3:, 5:-1, :, :] = 1
    film[:, 2:5, :, :] = 1
    scal = np.sum(film) * 1.0 / film.size
    affine = np.eye(4)
...
def test_ts_diff_ana_null():
    ...
    assert_array_almost_equal(report["volume_means"], scal * np.ones(n_scans))
    assert_array_almost_equal(report["volume_mean_diff2"], np.zeros(n_scans - 1))
    assert_array_almost_equal(
        report["slice_mean_diff2"], np.zeros((n_scans - 1, shape[2]))
    )
    assert_array_almost_equal(report["diff2_mean_vol"].get_fdata(), np.zeros(shape[:3]))
    assert_array_almost_equal(
        report["slice_diff2_max_vol"].get_fdata(), np.zeros(shape[:3])
    )
...
def test_ts_diff_ana():
    ...
    assert_array_almost_equal(report["volume_means"], scal * np.arange(n_scans))
    assert_array_almost_equal(report["volume_mean_diff2"], scal * np.ones(n_scans - 1))
    assert_array_almost_equal(
        report["slice_mean_diff2"], scal * np.ones((n_scans - 1, shape[2]))
    )
    assert_array_almost_equal(report["diff2_mean_vol"].get_fdata(), ref[..., 0])
    assert_array_almost_equal(report["slice_diff2_max_vol"].get_fdata(), ref[..., 0])
...
def test_ts_diff_ana_two_session():
    ...
    assert_array_almost_equal(
        report["volume_means"], np.tile(scal * np.arange(n_scans), 2)
    )
    assert_array_almost_equal(
        report["volume_mean_diff2"], np.tile(scal * np.ones(n_scans - 1), 2)
    )
    assert_array_almost_equal(
...
def test_ts_diff_ana_nan():
    ...
    assert_false(np.isnan(report["diff2_mean_vol"].get_fdata()).any())
    assert_false(np.isnan(report["slice_diff2_max_vol"].get_fdata()).any())


def test_issue_144():
    # tsdiffana should error on 3D images
    sd = _make_sd(
        func_filenames=["/tmp/titi/func1.nii"], func_ndim=3, output_dir="/tmp"
    )
    sd.sanitize()
    try:
